...
<div class="form-body">
    ...
    <div class="row gx-3">
        <div class="col-md-6">{!! BootForm::text(__('Tag'), 'tag')->required() !!}</div>
            ...
            <div class="input-group">
                {!!
                    Form::text('slug')
                        ...
                        ->addClass($errors->has('slug') ? 'is-invalid' : '')
                        ->id('slug')
                        ->data('slug', 'tag')
                !!}
    ...
    @if ($model->id)
                            ...
                            @foreach ($items as $item)
                                <li class="list-group-item d-flex gap-2 align-items-center px-0">
                                    @if (method_exists($item, 'editUrl'))
                                        <a class="btn btn-xs btn-light" href="{{ $item->editUrl() }}">{{ __('Edit') }}</a>
                                    @endif
                                    <div>
        ...
        @else
            <div class="alert alert-info mt-4">{{ __('No items are tagged with this tag yet.') }}</div>
        @endif
    @endif
</div>
